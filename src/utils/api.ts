...
// Función para editar un usuario en Auth0
export async function updateAuth0User(
  userId: string,
  updatedData: { email?: string; name?: string; password?: string },
  accessToken: string,
  tokenType: string
) {
  ...
  if (!response.ok) {
    console.error("Error editando el usuario en Auth0:", await response.text());
    return null;
  }
...
export async function deleteAuth0User(
  ...
  if (!response.ok) {
    console.error(
      "Error eliminando el usuario en Auth0:",
      await response.text()
    );
    throw new Error("Error eliminando el usuario en Auth0");
  }
